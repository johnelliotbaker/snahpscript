	...
	*/
	function send($method = NOTIFY_EMAIL, $break = false)
	{
		...
		*/
        ...
        $message = $this->msg;
        $aAllowedPattern = [
            // User registration and resending activation
            '/\s+Please visit the following link in order to activate your account:/',
            // forgot my password
            '/.*If successful you will be able to login using the following password:.*/',
        ];
        $break = true;
        foreach($aAllowedPattern as $k => $ptn)
        {
            preg_match($ptn, $message, $match);
            if ($match)
            {
                $break = false;
                break;
